...
class IMG_Stegno:
    ...
    def main(self, root):
        root.title('WELCOME TO PICTURE CRYPTO')
        ...
        title = Label(frame,text='WELCOME TO PICTURE CRYPTO')
    ...
    def encode_frame1(self,F):
        F.destroy()
        F2 = Frame(root)
        label1= Label(F2,text='Select the PICTURE in which \nyou want to insert SECRET MESSAGE:')
        ...
        button_bws.grid()
        button_back = Button(F2, text='CANCLE', command=lambda : IMG_Stegno.back(self,F2))
    ...
    def decode_frame1(self,F):
        F.destroy()
        d_f2 = Frame(root)
        label1 = Label(d_f2, text='Select PICTURE with SECRET MESSAGE:')
        label1.config(font=('Times new roman',25,'bold'),bg = '#e3f4f1')
        label1.grid()
        ...
        button_bws.config(font=('Helvetica',18), bg='#e8c1c7')
        button_bws.grid()
        button_back = Button(d_f2, text='CANCLE', command=lambda : IMG_Stegno.back(self,d_f2))
        button_back.config(font=('Helvetica',18), bg='#e8c1c7')
        button_back.grid(pady=15)
        button_back.grid()
        d_f2.grid()
    ...
    def encode_frame2(self,e_F2):
        ...
        if not myfile:
            ...
            label3= Label(e_pg,text='SELECTED PICTURE')
            ...
            board.grid()
            label2 = Label(e_pg, text='ENTER THE SECRET MESSAGE')
            label2.config(font=('Helvetica',14,'bold'))
            label2.grid(pady=15)
            text_a = Text(e_pg, width=50, height=10)
            text_a.grid()
            encode_button = Button(e_pg, text='CANCLE', command=lambda : IMG_Stegno.back(self,e_pg))
    ...
    def decode_frame2(self,d_F2):
        ...
        if not myfiles:
            ...
            label4= Label(d_F3,text='SELECTED PICTURE :')
            ...
            label2 = Label(d_F3, text='SECRET MESSAGE is :')
            label2.config(font=('Helvetica',14,'bold'))
            label2.grid(pady=10)
            ...
            text_a.grid()
            button_back = Button(d_F3, text='CANCLE', command= lambda :self.frame_3(d_F3))
            button_back.config(font=('Helvetica',14),bg='#e8c1c7')
            button_back.grid(pady=15)
            button_back.grid()
            d_F3.grid(row=1)
    ...
    def enc_fun(self,text_a,myImg):
        data = text_a.get("1.0", "end-1c")
        if (len(data) == 0):
            messagebox.showinfo("Alert","Kindly ENTER text in SECRET MESSAGE")
        else:
            newImg = myImg.copy()
